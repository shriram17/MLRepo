...
# Function designed to split the Hash Tag content in most likely form
# in the dictionary using Viterbi alogorithm
...
dictionary = dict((w, len(list(ws)))
                  for w, ws in groupby(sorted(words(open('words.txt').read()))))
max_word_length = max(map(len, dictionary))
total = float(sum(dictionary.values()))


############################################################################
#Function to Replace SMS lingo words using SMS lingo dictionary 
# and dictionary of corrected words
...
def smslingo(message):
    ...
    f1 = open('dictionary1.txt',"r+")
    f2 = open('dictionary2.txt',"r+")
    f3 = open('count.txt',"r+")
    f5 = open('label.txt',"r+")
...
#SVM classification
def svmfunction(train,test):
    samples = csv.reader(open(train, 'rt'), delimiter=',', quotechar='|')
    param = svm_parameter()
    param.C = 10
    ...
    test_data = csv.reader(open(train, 'rt'), delimiter=',', quotechar='|')
